...
from simreaduntil.shared_utils.utils import delete_dir_if_exists, dill_dump, dill_load, print_args
...
from simreaduntil.simulator.simulator import ONTSimulator, convert_action_results_to_df, run_periodic_mux_scan_thread, simulator_stats_to_disk, stop_simulation_after_time_thread
...
def get_reads_writer(run_dir: Path, rotating: bool):
    ...
    if rotating:
        ...
        reads_writer.output_dir = mk_run_dir
...
def get_sim_params(sim_params_file, n_channels) -> SimParams:
    if sim_params_file is None:
        ...
        sim_params = SimParams(
            gap_samplers={f"{i+1}": ConstantGapsUntilBlocked(short_gap_length=0.4, long_gap_length=10.1, prob_long_gap=0, time_until_blocked=np.inf, read_delay=0) for i in range(n_channels)},
            bp_per_second=450, chunk_size=200, default_unblock_duration=0.1, seed=0,
        )
    else:
...
def get_read_pool(reads_file, ref_genome_path):
    ...
    if reads_file is None:
        ...
        read_pool = ReadPoolFromIterable(perfect_reads_gen(ref_genome_path, read_lens_range=(5_000, 10_000), random_state=np.random.default_rng(1)))
    else:
...
def main(toml_file):
    ...
    rotating = config.get("rotating", True)
    ...
    assert readfish_method in ["unblock_all", "targeted_seq"]
    ...
    if (ref_genome_path is not None) and (reads_file is not None):
        logger.info(f"You will reach average coverage {compute_nonselective_coverage(ref_genome_path, reads_file=reads_file)}")

    if run_dir.exists():
        logger.warning(f"Run dir '{run_dir}' already exists")
    run_dir.mkdir(exist_ok=True)
    reads_writer = get_reads_writer(run_dir, rotating=rotating)
    read_pool = get_read_pool(reads_file, ref_genome_path=ref_genome_path)
    ...
    try:
        with wrap_simulator_in_grpc(simulator, use_grpc=use_grpc):
            if readfish_method == "unblock_all":
    ...
    if isinstance(reads_writer, ArrayReadsWriter):
        ...
        assert isinstance(reads_writer, (SingleFileReadsWriter, RotatingFileReadsWriter))
