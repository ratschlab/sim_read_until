"""
Manage simulation parameters such as bps_per_second, min_chunk_size, gap_samplers
"""
...
class SimParams:
    ...
    Args:
        ...
        min_chunk_size: minimum chunk size for selective sequencing ReadUntil
        seed: seed for random number generator, same state set on all channels
    """
    def __init__(self, gap_samplers: Dict[str, GapSampler], bp_per_second=450, default_unblock_duration=0.1, min_chunk_size=200, pore_model: Optional[PoreModel]=None, seed: Union[int, np.random.Generator]=0):
        self.set(gap_samplers=gap_samplers, bp_per_second=bp_per_second, default_unblock_duration=default_unblock_duration, min_chunk_size=min_chunk_size, seed=seed, pore_model=pore_model)

    def restrict_to_channels(self, channels, rand_state):
        """Subset SimParams to some channels"""
        return SimParams(gap_samplers={channel: self.gap_samplers[channel] for channel in channels}, bp_per_second=self.bp_per_second, default_unblock_duration=self.default_unblock_duration, min_chunk_size=self.min_chunk_size, seed=rand_state)
        
    def __repr__(self) -> str:
        # repr(random_state) is not very informative (does not show seed, so we store it separately and display it here)
        return f"""SimParams(bp_per_second={self.bp_per_second}, default_unblock_duration={self.default_unblock_duration}, min_chunk_size={self.min_chunk_size}, initial_seed={self._initial_seed}, n_channels={len(self.gap_samplers)})"""

    def set(self, *, gap_samplers: Dict[str, GapSampler]=None, bp_per_second=None, default_unblock_duration=None, min_chunk_size=None, pore_model=None, seed=None):
        """
    ...
    def _check_sim_params(self):
        ...
        assert isinstance(self.min_chunk_size, int)
